...
Set `ACOB_SRV_DATA_DIR` to move the SQLite fallback file and locally hosted
captures. Set
`ACOB_SRV_PUBLIC_URL` when capture download URLs need an origin other than the
incoming request host. Compose sets it to `http://127.0.0.1:${ACOB_PROXY_PORT}` because
...
1. `ACOB_SRV_DATABASE_URL`, e.g.
   `postgres://acob:acob@acob-db:5432/acob`;
2. Postgres from `ACOB_SRV_DB_HOST` plus `ACOB_SRV_DB_NAME` / `ACOB_SRV_DB_USER` /
   `ACOB_SRV_DB_PASSWORD` / `ACOB_SRV_DB_PORT` (Compose sets host `acob-db`, port
   `5432`, name/user/password from `ACOB_DATABASE_PORTGRES_*`);
...
| --- | --- | --- |
| `ACOB_SRV_DATABASE_URL` | unset (SQLite fallback) | Single Postgres URL (preferred for overrides). |
| `ACOB_SRV_DB_HOST` / `ACOB_SRV_DB_PORT` | unset / `5432` | Postgres host/port; setting `ACOB_SRV_DB_HOST` selects Postgres. |
| `ACOB_SRV_DB_NAME` / `ACOB_SRV_DB_USER` / `ACOB_SRV_DB_PASSWORD` | `acob` / `acob` / `acob` | Postgres credentials (Compose wires `ACOB_DATABASE_PORTGRES_*`). |
| `ACOB_SRV_DEBUG` | `true` | Django debug mode; Compose sets `ACOB_SRV_DEBUG=false`. |
| `ACOB_SRV_SECRET_KEY` | dev fallback (DEBUG only) | Required when `DEBUG` is false; boot fails without it. |
| `ACOB_SRV_ALLOWED_HOSTS` | `*` | Allowed hosts list (CSV). |
| `ACOB_SRV_CSRF_TRUSTED_ORIGINS` | unset | Trusted origins list (CSV). |
| `ACOB_SRV_DATA_DIR` | repo `srv/` | SQLite fallback + media root parent. |
| `ACOB_SRV_PUBLIC_URL` | unset (request origin) | Origin for capture download URLs. |
...
```bash
ACOB_SRV_SUPERUSER_USERNAME=admin \
ACOB_SRV_SUPERUSER_EMAIL=admin@example.com \
ACOB_SRV_SUPERUSER_PASSWORD=changeme \
make dev
```
...
the password to the env value. When `ACOB_SRV_SUPERUSER_PASSWORD` is unset it
...
`make install ACOB_PROXY_PORT=... ACOB_APPLICATION_NAME=...` requires `ACOB_APPLICATION_NAME` and uses installation context
and project `acob-<port>-<name>`. For example,
`make install ACOB_PROXY_PORT=61554 ACOB_APPLICATION_NAME=alexandro` uses `acob-61554-alexandro` and starts
a managed browser with a project-scoped profile. `ACOB_APPLICATION_NAME` may
...
Host and port, independently of `ACOB_SRV_PUBLIC_URL`. nginx preserves Host,
...
- `DEBUG` defaults to enabled for native dev (`ACOB_SRV_DEBUG`);
  Compose sets `ACOB_SRV_DEBUG=false`. `ALLOWED_HOSTS` accepts every host by
