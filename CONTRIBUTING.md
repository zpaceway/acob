...
`acob-58346-default` with a managed browser and persistent profile. `NAME`
...
- `compose.yaml` is the only Compose file; it defines the `acob-srv`,
  `acob-mcp`, `acob-proxy`, and `acob-browser` services.
